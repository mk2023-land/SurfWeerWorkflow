...
1. **`research_gap_analysis.md`** (4411 woorden) — Per-case gap-analyse Tobias' 13 historische SMSes + woensdag 20 mei
...
Plus eerder onderzoek: `research_wave_physics_benchmark.md`, `research_tobias_methodology.md`, `research_benchmark_comparison.md`.

---

## 1. Executive Summary

Het huidige systeem matcht Tobias' woensdag-output op **2 van 3 windows** (12-14u en 18-21u) maar genereert nog een extra morning-window (06-09u) dat Tobias niet noemt. Diepere benchmarks op Tobias' 13 historische SMSes laten zien dat de overblijvende gap **drie families** heeft:

**A. Enkelvoudige wind-data** — één model (Open-Meteo `knmi_seamless`) terwijl Tobias 4 modellen triangulateert. **Belangrijkste bottleneck.**
...
Open-Meteo `knmi_seamless` is één getal per uur per grid-cel (~5 km). Tobias gebruikt Harmonie 2.5km + ECMWF + KNMI menselijke guidance. Wat dit oplevert dat ik mis:
...
Open-Meteo Marine geeft één swell-partitie + één wind-zee partitie. Tobias leest 2D-spectra met 2-3 aparte pieken. Surfline rolt nu actief "Swell Spectra" feature uit als premium feature.
...
Tobias' "ochtend rustig, opbouwend" is impliciete kennis hiervan. Wetenschappelijke proxy: wave age = c_p/U10:
...
Surfline doet hourly buoy assimilatie, Stormsurf vertrouwt primair op live buoys, Tobias checkt elke ochtend IJG1 spectrum, academic top-1 is XGBoost op buoy-residuals. **Ik gebruik buoys nergens voor correctie.**
...
Tobias kijkt naar drukkaarten, frontpassages. Mijn `pressure_msl` wordt opgehaald maar niet in scoring gebruikt. Drukgradiënt-derivative (`|dp/dt| > 1.5 hPa/uur` = frontpassage) zou veel automatisch detecteerbaar maken zonder extra API.

### R7. Deterministische output i.p.v. probabilistisch (genoemd in 3/4)
Tobias zegt "kan zaterdag wat zijn" (uncertain), Surfline rapporteert ranges, ECMWF ensemble heeft P10/P90. Ik geef één getal. Hierdoor verliest het systeem **operationele beslissingswaarde** voor de gebruiker bij twijfelgevallen.
...
| **2** | **Wave-age proxy** `cp/U10` als pre-filter | Pro-forecaster mechanism 6, academic top-3. Wind-sea met cp/U10 < 0.83 = pure chop. Tobias' "5s minimum ongeacht hoogte" is empirische proxy. | Tp en U10 al beschikbaar | XS (1u) | Hoog (filtert ochtend-spin-up correct) | 1 |
...
| **4** | **Wind-gust ratio** als instabiliteits-flag | Gap-analyse Gap 3. `gust/sustained > 1.5` = vlagerig (post-front, instabiel). Tobias' "vlagerig" wordt automatisch gedetecteerd. | `wind_gusts_10m` al opgehaald, ongebruikt | XS (1u) | Middel (2-3 cases) | 1 |
...
| **12** | **Diurnal wind-decay** rond zonsondergang | Pro-forecaster mechanism 8. Empirische regel: `als sunset_h − 2 ≤ uur ≤ sunset_h + 1 én cloud_cover < 50%: wind -= 2-3 kn`. Modelleert Tobias' "na 19:30 wind valt weg" effect. | Sunset-tijd berekenbaar, cloud_cover al opgehaald | S (½ dag) | Middel (verbetert avond-window detectie) | 2 |
...
| **17** | **Probabilistische output met model-spread** | Gap-analyse Gap 10, pro-forecaster mechanism 12. P25-P75 range ipv single getal in LLM-input. Overlap met #8 (multi-model). LLM mag dan "1.0-1.4m" of "modellen nog onzeker" schrijven. | Multi-model uit #8 | S (½ dag) | Middel (verbetert vertrouwen + Tobias-fit van taal) | 3 |
...
**Acceptance check tegen Tobias' 13 SMSes:** verwacht 4-6 cases verbeterd. Specifiek: CASE 1 (woensdag morgen wordt correct uitgesloten), CASE 4 (groundswell-event correct als ALERT), CASE 13 (vandaag correct flat).
...
**Acceptance:** ~7-8 cases binnen tolerantie van Tobias (was ~5).
...
- [ ] Verification framework met McNemar + Diebold-Mariano tests tegen Tobias

**Test-criterium:** wetenschappelijk gevalideerde 20-25% RMSE reductie aanhoudend over 90+ dagen. HSS go/no-go classification > 0.60 vs Tobias als baseline.
...
|---|---|---|---|---|---|
| Cases binnen Tobias-tolerantie | 5/13 | 7/13 | 8-9/13 | 10-11/13 | 11-12/13 |
...
| Output-stijl (Tobias-lijkenheid) | 70% | 75% | 80% | 85% | 85% |
...
1. **Spot-specific sandbank kennis.** Vereist 1+ jaar eigen waarneming. Tobias' "Wijk werkt op X, Noordwijk op Y" blijft buiten bereik.
2. **KNMI menselijke guidance.** Tobias leest een geredigeerde menselijke synthese die niet als API beschikbaar is. We benaderen dit via multi-model spread maar 100% replicatie is onmogelijk.
3. **Subjectieve voorkeur ("ochtend nooit, middag altijd").** Sommige van Tobias' picks zijn persoonlijke conventie. Zonder die mining van zijn historische SMSes blijft mijn output objectiever (= soms onverwachte morning-windows).
...
| Risico | Mitigatie |
|---|---|
| Sprint 1 fixes leveren stapeling van penalty's → alle scores nul | Stage-gate test na elke fix: ten minste 2 van Tobias' cases moeten blijven scoren als surfable. Bij regressie: tune-down tot acceptabel. |
...
Per fix: implementeer, run dry-run-benchmark op woensdag-20-mei case, vergelijk met Tobias. Commit per fix met test-case bewijs.

Na Sprint 1: tweede benchmark-ronde. Als die laat zien dat ochtend-window nu correct uit Tobias-pattern valt, **dan pas Sprint 2** beginnen. Anders eerst Sprint 1-fine-tuning.
...
Het onderzoek over 4 onafhankelijke sporen (industry, pro-forecaster, academic, gap-analyse Tobias) convergeert op 7 systemische gaps die ALLEMAAL terugkomen, met **multi-model wind-triangulatie + partition-aware scoring + buoy bias-correctie** als de drie absolute hoofdthema's.
...
De realistische ambitie: na Sprint 1+2 (max 5 dagen werk) zit het systeem op **10/13 cases binnen Tobias-tolerantie**. Na Sprint 3 op **11/13**. Volledig Tobias-niveau vereist Sprint 4 (XGBoost op 6+ maanden Meetpost Noordwijk data) — niet realistisch deze maand maar wel binnen 6 maanden bereikbaar.

De honest cap: zelfs perfect uitgevoerd haalt het systeem geen 13/13 omdat sommige van Tobias' picks subjectief zijn (lokale conventie, persoonlijke voorkeuren) of leunen op spot-specifieke bank-kennis die zonder jaren-lange waarneming niet replicabel is.
