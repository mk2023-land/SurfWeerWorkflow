...
Gebruik (via stdin pipe):
    cat msg_today.txt | python scripts/ingest_tobias_message.py --date 2026-05-20

Bestand-layout (in de private archive-repo ~/Merlijn/Tobias):
    data/tobias_archive/
...
# Tobias-referentie-archief leeft sinds 2026-05-22 in een aparte private repo
# (auteursrechtelijk materiaal): ~/Merlijn/Tobias. Default verwacht die naast
# de hoofdrepo; override met de TOBIAS_ARCHIVE_DIR env-var voor een andere locatie.
_default_archive = (
    Path(__file__).resolve().parent.parent.parent
    / 'Tobias' / 'data' / 'tobias_archive'
)
...
# Verdict-keywords (Tobias' lexicon, zie research/tobias_methodology.md §4.3)
VERDICT_KEYWORDS = {
...
# Windgegevens uit Tobias' tekst (bv. "5bft", "tot 4bft", "ZW")
...
def main():
    ...
    if args.text:
        ...
        if sys.stdin.isatty():
            print(f"→ Plak Tobias' SMS voor {msg_date.isoformat()}, druk Ctrl+D wanneer klaar:")
        text = sys.stdin.read()
