...
**Scope:** industry leaders (Surfline, Magicseaweed, Stormsurf, Surf-Forecast.com, Windguru) plus Europese/NL-specifieke diensten (Surfana, Ridersguide, Seven at Sea, Surfweer.nl, Goedegolven, Surf-Report.com).
...
### 6.4 Surfweer.nl (Tobias van Tellingen)

Tobias gebruikt **een sterk subjectieve narrative-stijl methodologie** met expliciete spectra-decompositie. Sleutelelementen uit zijn publieke posts:

- **Spectra-analyse via Rijkswaterstaat-boeien**: hij linkt naar surfweer.nl/spectra met click-baar boei-veld (Munitiestort, Maasgeul, etc.) en leest periode én richting handmatig uit het 2D-spectrum (b.v. "140 mHz uit WZW = 7.1s uit 250°").
...
Bronnen: [Surfweer.nl spectra-pagina](https://surfweer.nl/surf/spectra/), [Tasha's Surfcamp: Tobias profiel](https://tashasurfcamp.com/tobias-de-piet-paulusma-van-het-surfen).
...
| Parameter | Surfline rating | MSW rating | Surf-Forecast | Stormsurf | Tobias |
|---|---|---|---|---|---|
...
- **Stormsurf/Tobias:** handmatig met spot-kennis.
...
- **Tobias:** wél in rating, via heuristieken (water-op-bank, stroming tegen swell, spring/doodtij).
...
- **Voor NL toepasselijk:** Rijkswaterstaat Munitiestort/IJmuiden/Eierland boeien zijn open data via waterinfo.rws.nl en publiceren spectra. Tobias gebruikt dit; eigen pipeline doet het nog niet.
...
| Aspect | Naive scoring (size-only) | Surfline (LOTUS + ML) | Magicseaweed | Surf-Forecast.com | Stormsurf | Tobias (Surfweer.nl) |
|---|---|---|---|---|---|---|
...
- **Wat industry doet:** Surfline (forecast spectra), Stormsurf (2D-buoy spectra), Tobias (RWS-spectra) gebruiken alle gescheiden swell-partities.
...
- **Wat industry doet:** Surfline laat tide bewust uit de model-rating en verlaat zich op forecaster-override. Tobias gebruikt expliciete heuristieken per spot. Surfana waarschuwt voor stroming verschillen.
- **Practical fix:** maak `tide_optimum_curve` per-spot configureerbaar i.p.v. globale formule. Voor Noordwijk: leer uit Tobias' posts (vaak laag tot mid op opkomend) en bouw aparte curves voor wind-swell (laag-water-bonus voor steepening) en groundswell (geen sterke voorkeur). Voeg `spring/doodtij` modifier toe via een eenvoudige tidal-range-check uit getij.nl.
...
- **Wat industry doet:** Surfline ML-laag leert dit impliciet; Surf-Forecast schaalt "in proportion to wind speed" relatief tot wave-strength; Stormsurf/Tobias doen dit handmatig.
...
- **Wat industry doet:** Surfline hourly model + buoy-assimilatie (30–40% error reduction door bias-correctie alleen). Stormsurf vertrouwt vooral live boei-data. Tobias check RWS-spectra elke ochtend.
...
- **Wat industry doet:** Surfline's "Wave Consistency" feature toont dit; Tobias schrijft regelmatig "dag na de storm" als bonus-window.
...
4. **Tide × bathymetrie is universeel onderbedeeld** in geautomatiseerde systemen; Tobias' handmatige heuristieken zijn hier de gouden standaard voor NL.
...
- **P6** (optioneel, hogere investering): trainen van een eenvoudige logistische regressie of MLP op historische Tobias-posts als labels, à la Surfline's neural net approach.
...
- [Tasha's Surfcamp: Tobias profiel](https://tashasurfcamp.com/tobias-de-piet-paulusma-van-het-surfen)
