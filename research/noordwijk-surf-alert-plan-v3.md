...
> - Diepe analyse van Tobias' methodiek op basis van 13 SMS'jes (juli–augustus 2025 + mei 2026 + september 2025) waarin hij expliciet uitlegt hoe hij denkt
...
> - IJgeul boei (IJG1) als primaire meting voor Noordwijk — Tobias noemt deze boei expliciet voor deze spot
...
1. Data ophaalt uit dezelfde bronnen die Tobias gebruikt (RWS-boeien inclusief 2D-spectra, KNMI Harmonie, ECMWF, GFS, UKMO via Open-Meteo)
...
**Doel**: alert-frequentie en alert-kwaliteit zo dicht mogelijk bij Tobias' werkwijze houden, voor 1 spot (Noordwijk).

---

## DEEL 1 — Hoe Tobias denkt (gereconstrueerd uit zijn SMS'jes)

### 1.1 Zijn data-stack (uit weerlinks-pagina en verwijzingen in SMS)

Tobias gebruikt — in volgorde van hoe hij ernaar verwijst in de SMS'jes:
...
Tobias rekent in **millihertz** en converteert naar seconden:
...
| Frequentie | Periode | Wat het betekent | Tobias' woorden |
|---|---|---|---|
...
### 1.5 Wanneer Tobias géén alert stuurt
...
| Datum | Status Noordwijk | Type | Tobias' redenering | Wat algoritme zou moeten zien |
|---|---|---|---|---|
...
Tobias noemt Noordwijk in deze SMS'jes meestal als deel van groepering "Katwijk/Nwijk/Zvoort" of "Wssnaar-Nwijk". Patronen voor Noordwijk-alert-momenten:
...
Belangrijk: één spectrum kan **2-3 pieken** hebben. Tobias' inzicht: een groundswell-piek (10s) + wind sea-piek (4s) tegelijk = mooie surf (de groundswell komt door).
...
```
Je schrijft korte surf-SMS'jes voor Noordwijk in de stijl van Tobias 
...
**Voor Noordwijk: IJG1 (IJgeul) is primaire boei**, want Tobias gebruikt deze expliciet voor Noordwijk/Zandvoort/Scheveningen.
...
| Surfweer.nl stijl te dichtbij overgenomen | LLM output is paraphrase, geen letterlijke quotes |
...
1. **2D-spectrum parser**: RWS levert ook richting-gefilterde spectra. Implementeer een echte FFT-deconstructie van wind_sea / wind_swell / ground_swell pieken met directionele info. Dit benadert Tobias' "spectra"-analyse beter dan de huidige gepartitioneerde data.
...
7. **Synoptische narratie**: voor multi-day outlook in digest: laat de LLM een 1-zinsverklaring genereren over wat er meteorologisch gebeurt ("hogedruk boven Schotland, geen swell verwacht"). Dit benadert Tobias' synoptische uitleg.

---

## Appendix A — Surfweer.nl bronnen die we direct meenemen

| Tobias' bron | Onze implementatie |
|---|---|
...
## Appendix B — Tobias' afkortingen en spotgroepen

```python
NEARBY_SPOTS = {
    'noordwijk':  ['Nwijk'],
    'cluster':    ['Katwijk', 'Nwijk', 'Zvoort', 'Wssnaar'],  # groepering Tobias
...
## Appendix C — Tobias' meteorologische verklaringen om in algoritme te verwerken
...
- `date`
- `tobias_alert_explicit` (boolean)
- `tobias_noordwijk_assessment` (text paraphrase)
- `tobias_alert_type` (een van T1-T5 of "none")
...
Dit plan benadert Tobias' werkwijze zo goed als algoritmisch mogelijk is voor één spot (Noordwijk). De LLM-laag zorgt voor natuurlijke berichten, de deterministische scoring zorgt voor reproduceerbare beslissingen, en de validatieset zorgt voor kwaliteitsborging tegen zijn historische output.

Het is geen vervanging van Tobias zelf — hij heeft 16+ jaar surferservaring en kan dingen zien die het algoritme niet ziet. Maar voor de specifieke use case "stuur me een SMS als Noordwijk binnen 24u goed wordt" is dit een sterke benadering.
...
5. **Genoeg historische SMS-data van Tobias om tegen te valideren** — als gebruiker meer kan aanleveren, wordt het systeem proportioneel beter
