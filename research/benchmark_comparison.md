# Benchmark: SurfWeerWorkflow vs. referentie-forecaster vs. internationale best practice

**Datum benchmark:** 19 mei 2026 (test-bericht voor woensdag 20 mei 2026)
**Doel:** vaststellen waar het systeem afwijkt van pro-forecaster de referentie-forecaster en van de internationale wave-physics literatuur, om gerichte verbeteringen te kunnen prioriteren.
**Bronnen:**
- `wave_physics_benchmark.md` — internationale wave-fysica & forecasting best practices (Surfline, Stormsurf, NDBC, ECMWF, etc.)
- `reference_methodology.md` — reverse-engineering van het beslissingsmodel van de referentie-forecaster + bericht-decompositie
...
> **Titel:** Surf-update Noordwijk van dinsdag 19 mei
>
...
| **14:00** | 1.12 | 4.5 | 247° | 16.3 / 236 | 0.26 | afgaand | 18.9 | 3.7 | 8.7 | 5 | **36.3** ← window 1 van de referentie-forecaster |
...
| **19:00** | 0.68 | 3.5 | 243° | 14.0 / 238 | 0.79 | afgaand | 13.4 | 6.5 | 18.0 | 5 | **42.9** ← window 2 van de referentie-forecaster |
...
**Surfbare windows (drempel 60): 0.** Geen ALERT gegenereerd — terecht, want de referentie-forecaster gaf ook geen alert. Maar de referentie-forecaster noemde wél twee bruikbare windows die mijn systeem volledig mistte.

## 3. Vergelijking van de claims van de referentie-forecaster met mijn data

Voor de twee windows die de referentie-forecaster expliciet noemt voor Noordwijk:

| Parameter | De referentie-forecaster zegt | Mijn data | Diagnose |
|---|---|---|---|
| **Window 14-16u** | | | |
...
**Diagnose:** de DATA is grotendeels correct (wind, hoogte, getij). Het is de **score-rekenkundige interpretatie** die te streng is voor de longboard-windows van de referentie-forecaster.
...
|---|---|---|---|
| **A1**: Hs binnen ±20% van pro forecast | 0.68-1.36m vs de referentie-forecaster ~1.0m | ✅ | binnen tolerantie |
| **A2**: Tp gerapporteerd in seconden (niet alleen Hs) | ja | ✅ | |
| **A3**: Tp binnen ±2s van pro | 4.0-5.8s vs de referentie-forecaster 5-7s | ⚠️ | ~1-2s structureel laag |
...
| **B3**: Periode-optimumcurve rond 7s (niet lineair "hoger = beter") | nee, lineaire score | ❌ | het "ideaal 6.5-7s voor NL" van de referentie-forecaster niet meegemodelleerd |
...
| **D3**: Tidal currents (vloed/eb) als negatieve factor | nee | ❌ | het "vloedstroom vol vanaf 15u" van de referentie-forecaster niet meegerekend |
...
| **F5**: Geen alert wanneer de referentie-forecaster geen alert doet | ja (correct) | ✅ | |
| **F6**: Surfable windows wanneer de referentie-forecaster zegt rideable | nee (0 vs 2) | ❌ | |
...
| 3 | Wind-scoring rebalance voor 12-22 kn range | HOOG (longboard-windows van de referentie-forecaster zichtbaar) | M | `src/scoring/hourly.py`, `src/config.py` |
...
Idealiter (op basis van de referentie-forecaster-stijl plus correcte data):

> **Surf-update Noordwijk van dinsdag 19 mei**
>
...
Wat hier anders is: (a) géén pre-dawn pieken, (b) golfhoogte = écht hoogste uur (10u = 1,4m), (c) **twee longboard-windows** expliciet genoemd, (d) bordtype-advies, (e) geen verzonnen "springtij", (f) lengte vergelijkbaar met van de referentie-forecaster werkelijke SMS.
...
3. De twee duidelijke longboard-windows van de referentie-forecaster volledig gemist
...
De referentie-forecaster zei: "**Nwijk/Zvoort 14-16u of na 19:30u**".
...
- 15-17u valt nu uit het venster door de vloedstroom-penalty — exact wat de referentie-forecaster bedoelt met "vol vloedstroom vanaf 15u".
...
| C2 | Wind 12-22 kn rideable goede richting | ❌ | ✅ | speed-curve veel gladder; van de referentie-forecaster 17 kn ZW scoort nu 11pt ipv 3pt |
...
| F4 | Twee windows op één dag detecteerbaar | ⚠️ | ✅ | tidal-current creëert nu de dip tussen de windows van de referentie-forecaster |
| F5 | Geen alert wanneer de referentie-forecaster geen alert | ✅ | ✅ | onveranderd |
| F6 | Surfable windows wanneer de referentie-forecaster rideable | ❌ | ✅ | 7 longboard windows ipv 0 |
...
- **Volgende benchmark dag**: opnieuw 24 uur na een echt bericht van de referentie-forecaster om calibratie verder te valideren over meerdere setups.
