...
├── ingest_tobias_message.py         # Tobias-referentieberichten archiveren als training-labels (private repo ~/Merlijn/Tobias)
...
- **Tobias-referentie-archief**: aparte private repo `~/Merlijn/Tobias` (user-geleverde SMS + parse-metadata, training-labels + `PATTERNS.md` Tobias-vs-systeem benchmark). Pad configureerbaar via `TOBIAS_ARCHIVE_DIR` env-var.
