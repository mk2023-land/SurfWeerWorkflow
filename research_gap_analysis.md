# GAP-analyse: SurfWeerWorkflow vs. Tobias' methodologie

**Doel:** voor elk structureel verschil tussen de output van het huidige Python-systeem en de SMS'en van Tobias van surfweer.nl precies vaststellen *wat* afwijkt, *waarom* het afwijkt, en *welke data of modelling* nodig is om het te dichten.
...
**Tobias' bericht:** "Nwijk/Zvoort 14-16u of na 19:30u. Genoeg hoogte." + "5 bft tot 20u, daarna afnemend tot 4 bft" + "Avond prima longboarden". Géén mentioning van ochtend.
...
3. Wind-gust ratio: bij gust/sustained > 1.5 = onstabiele luchtmassa, penalty -5pt op wind_score (Tobias' "vlagerig").

### CASE 2 — Zaterdag 16 mei 2026 (smal-alert)

**Tobias:** "Zvoort/Nwijk heel even 11-12u zonder wind" — alert-waardig vanwege windstilte-window middenin een drukke dag. Score-doel volgens validatie-tabel: 70-80 in smal window, ALERT (T5+T3).
...
**Gap: de +1pt bonus is veel te bescheiden.** Tobias' alert-waardigheid voor T4 is *exact* dit fenomeen. De huidige config heeft alleen +1pt voor groundswell-through-windsea — dat brengt een score van 70 naar 71, niet alert-waardig. Een T4-bonus moet ~+10pt zijn met als voorwaarde:
- swell_height ≥ 0.7m
- swell_period ≥ 9s
...
**Tobias:** "Nieuwe N-swell op 140mhz" — d.w.z. 7.1s. Plus windafname laat op de dag. Score-doel: 70-85 avond, mogelijk ALERT.
...
**Tobias:** "Koufront passeert vanuit het noorden, NNO naar ONO" — wind draait naar offshore. Score-doel: 70-80 in 2u window, ALERT.
...
**Tobias:** mediocre. Score-doel: 55-70, GEEN alert.
...
**Tobias:** wind-dip avond. Score-doel: 60-75 avond.
...
**Tobias:** 1,5m swell uit N op 10sec. Score: 75-90 ALERT.
...
**Tobias:** "Krachtige W/WZW 's nachts genereert golven, windafname overdag". Score: 50-65 middag.
...
**Tobias:** "Vandaag flat, 0,4m wind-zee, ZZO 5kn aflandig, doodtij". Score: <20.
...
Het systeem gebruikt één wind getal per uur uit Open-Meteo `knmi_seamless`. Tobias gebruikt:
...
- Hoeklokale variabiliteit (Z-H vs N-H kan 30-60° verschillen, Tobias zegt "Wind meer WZW in Z-H en Zeeland")
...
Open-Meteo Marine geeft één `swell_wave_*` partitie. Tobias leest 2D spectra met 2-3 aparte pieken inclusief richting per piek. Wat het systeem mist:
...
Tobias' intuïtie hiervoor: "ochtend nog rustig, opbouwend door de dag" — hij waardeert pas wave-velden ná hun groeifase.

### Type R5 — Synoptische context (CASE 1, CASE 6, alle alert-cases)

Tobias kijkt naar UKMO drukkaarten, ECMWF pluim, KNMI guidance om te begrijpen *waarom* een specifieke setup ontstaat:
...
**Impact:** middel-hoog. Voor Tobias' ochtend-skip op CASE 1 zou dit een 10-15pt drop geven, exact waar nodig. Detecteert ook automatisch frontpassages waar modellen uiteenlopen. Schatting: 4-6 van 13 cases verbeterd.

### Gap 2 — Wave-age proxy via Hs-derivative

**Wat:** Bereken `Hs_growth_rate = (Hs[t] - Hs[t-4]) / 4`. Bij groei > 0.15 m/uur (snelle wave-build) pas multiplier 0.85 toe op golf_score. Bij groei <0 (afzwakkend veld) en Hs > 0.7m: pas multiplier 1.05 toe (Tobias' "wind valt weg, swell loopt door").
...
**Impact:** hoog voor N-swell cases. Voor CASE 3 (N-deining 1m) en CASE 11 (1,5m N-swell): voorkomt false-negative van "blocked sector" terwijl Tobias een ALERT verstuurt. Schatting: 2-3 cases verbeterd, maar deze cases zijn de meest impactvolle (alerts!).
...
**Impact:** laag-middel. Verbetert de "kwalitatieve" tone-fit met Tobias' eigen voorbehouden ("kan zaterdag wat zijn"). Geen quantificeerbare case-impact maar belangrijk voor user-trust. Schatting: 2-3 cases krijgen betere bericht-tekst.
...
- `research_tobias_methodology.md` (intern, reverse-engineering Tobias' methode)
...
Het systeem heeft na blok 1+2 een sterke basis: scoring is goed gekalibreerd, daglicht-filter werkt, anti-hallucinatie werkt, longboard-threshold dekt Tobias' "matige longboard-dag" patroon. Wat structureel overblijft zijn **drie families van gaps**:

1. **Wind-modellering enkelvoudig** (R1, R5): één model i.p.v. multi-model triangulatie zoals Tobias doet. Oplosbaar binnen 1-2 dagen door Open-Meteo's multi-model endpoint te benutten.
...
De top-4 fixes (Sprint 1) kosten samen ~1 dag werk en zouden de top-3 falende cases (woensdag 20 mei ochtend-skip, CASE 4 groundswell-event, CASE 11 N-swell alert) tot binnen Tobias' tolerantie brengen. Daarna is het systeem voor de Noordwijk-spot specifiek een sterke benadering van Tobias' methodiek voor 90%+ van de cases.
